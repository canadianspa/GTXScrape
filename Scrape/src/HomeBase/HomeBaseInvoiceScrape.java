...
public class HomeBaseInvoiceScrape extends Application {
					...
					public void changed(ObservableValue ov, State oldState, State newState) {
						...
						else if(tabWebPos.get(place) ==6)
						{
							...
							if(isHomeBase(html))
							{
								...
								Invoices myEDA;
								try {
									myEDA = HomeBaseInvoiceScrape.this.readInvoices(html);
	...
	public Invoices readInvoices(String html) throws Exception
	{
		...
		html = html.replace("\n", "").replace("\r", "");
		invoiceNo = html.substring(html.indexOf("Invoice No") + 84, html.indexOf("<", html.indexOf("Invoice No")+84));
		System.out.println(invoiceNo);
		invDate = html.substring(html.indexOf("Invoice Date") + 86, html.indexOf("<", html.indexOf("Invoice Date")+86));
		invDate = invDate.replace(".", "/");
		System.out.println(invDate);

		return new Invoices(invoiceNo,invDate);
	...
	public static void main(String[] args) {
		...
		HomeBaseInvoiceScrape.launch(myArg);
